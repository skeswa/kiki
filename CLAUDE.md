...
The canonical contract for what this repo is building lives across the docs tree, not in any one file. Start at `docs/README.md` for navigation, then read in this order based on what you're doing:

- For behavior questions (what does `kk x` do?): `docs/specs/invariants.md` first, then the relevant `docs/specs/*.md`.
- For structure questions (which crate, which sqlite table, which gRPC method?): `docs/architecture/*.md`.
- For "why is it this way?": `docs/decisions/*.md` for ADR-style rationale; `docs/decisions/rejected.md` for closed alternatives.
- For deferred surfaces (v2 MCP substrate, etc.): `docs/roadmap/*.md`.
- For terminology: `docs/glossary.md`.
- For product intent and v1 scope: `docs/product/overview.md`.

`docs/prds/0001-kiki.md` is the original PRD. It is now **frozen** — its load-bearing content has been migrated into the spec / architecture / decisions / roadmap / glossary tree. Read it for historical context only; treat the live tree as the contract. New requirements ship as new PRDs (`0002-…`) describing deltas, or directly as edits to the spec / architecture files.
...
PRDs and specs in this repo are expected to survive a Codex review pass. The first PRD's review found three v1-scope contradictions (config layering, Codex harness, `kk reopen`), all resolved in the same revision. When editing specs, architecture docs, or new PRDs, cross-check against `docs/specs/invariants.md` and against neighboring specs before publishing.

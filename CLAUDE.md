...
The canonical contract for what this repo is building lives in the reference book under `docs/book/`. Start at `docs/README.md` for navigation, then read in this order based on what you're doing:

- For behavior questions (what does `kk x` do?): `docs/book/03-invariants.md` first, then the relevant chapter in `docs/book/`.
- For structure questions (which crate, which sqlite table, which gRPC method?): `docs/book/14-architecture/`.
- For "why is it this way?": `docs/appendix/decisions/` for ADR-style rationale.
- For deferred surfaces (v2 MCP substrate, etc.): `docs/book/17-roadmap.md`.
- For terminology: `docs/book/02-glossary.md`.
- For product intent and v1 scope: `docs/book/00-abstract.md` and `docs/book/01-orientation.md`.

`docs/appendix/prds/0001-kiki.md` is the original PRD path retained as a historical stub. Its load-bearing content has been folded into the reference book. Treat the book as the contract.

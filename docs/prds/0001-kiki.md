...
- `repos`, `threads` (with `cascade_seq` column), `thread_links` (follows edges), `agent_sessions` (with `last_seen_seq`, `session_id` for `--resume`), `context_queue` (PreToolUse-drained messages), `metadata_writes` (kk-ownership content-hash ledger for descriptions and bookmark names), `cascades` (in-flight rebase coordination), `op_attribution` (kk-initiated op-id dedupe), `pr_links` (thread → PR), `thread_config` (per-thread config layer key/value/updated_at).

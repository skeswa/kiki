...
| Repo summaries     | `RepoThreadSummaries` (server-streaming)                   | `ThreadScoped<T>` (read-only sibling summaries, same-repo only — see [`../specs/auth.md`](../specs/auth.md)) |
...
There is **no** `Switch` RPC. `kk switch <thread>` is not a daemon mutation — `kkd` does not own focus. The CLI decomposes switch into (i) a read-only `RepoThreadSummaries`-class lookup of the target thread's tmux session name (any valid credential, including `ThreadScoped<other>`, suffices) and (ii) a `tmux switch-client -t <session>` invocation that targets tmux, not `kkd`. See [`../specs/auth.md`](../specs/auth.md).

(The MCP transcript surface, when it ships, lives on a separate socket — see [`../roadmap/mcp-v2.md`](../roadmap/mcp-v2.md).)

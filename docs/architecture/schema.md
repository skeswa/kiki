...
The list below is table-level. Column-level definitions are derived from spec rules in [`../specs/`](../specs/).
...
- `context_queue` — cascade messages with monotonic seq numbers per thread; drained at the next PreToolUse after delivery, not at delivery itself.
- `cascade_outbox` — per-(thread, applied_cascade_seq) row carrying `(payload, anchor_change_id, anchor_commit_id, anchor_op_id, prepared_at, delivered_at NULL, transcript_row_id NULL)` with `UNIQUE (thread_id, applied_cascade_seq)`. Pins synthetic payload + anchor at compose time. See [`../decisions/cascade-outbox.md`](../decisions/cascade-outbox.md).
...
- `pending_kkd_prepends` — TTL-bounded `(prepend_id PRIMARY KEY, thread_id, text_sha256, inserted_at)`. **No** `UNIQUE` on `(thread_id, text_sha256)` — duplicates are deliberately allowed and consumed FIFO. Match key omits `session_id` so fresh-session reopens match correctly.

### v2-only

- `causal_chains`, `causal_chain_visits` — deferred until the broader MCP substrate ships. See [`../roadmap/mcp-v2.md`](../roadmap/mcp-v2.md).
...
- `cascade_outbox`: `delivered_at IS NULL` distinguishes outstanding deliveries from completed ones; the lookup criterion `applied_cascade_seq > acknowledged_cascade_seq` deliberately ignores `delivered_at`.

## Migrations

Every change ships as a migration. Forward-only; breaking-schema migrations are not allowed once a release is cut. The migration runner is part of `kiki-core`.

...
- `kk-hook` degrades to pass-through when `kkd` is unreachable. The hook attempts a connection with a 200ms connect timeout and a 1s overall budget; if either expires, it returns Claude Code's `continue` decision so the agent's tool call is not blocked, then appends a structured failure record to `<workspace>/kiki-errors.log`. Cascade delivery is deferred: the pending state lives in `pending_cascade_seq` and `context_queue` (when the watcher saw the trigger before the outage) or is reconstructed from jj's op log by the op-log catch-up at restart. `cascade_outbox` is not the holding place for pending cascades — it only holds applied-but-not-yet-acknowledged ones. The next successful PreToolUse round-trip after `kkd` returns applies the rebase and delivers the synthetic payload. The cascade-safety invariant in [Invariants](../04-invariants.md) is preconditioned on `kkd` being reachable; pass-through is the precondition's failure mode, not a violation. See [Harness adapter](harness-adapter.md).

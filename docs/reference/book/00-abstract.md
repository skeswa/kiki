...
Its unit is a **thread**: a line of work with its own jj workspace, live head at that workspace's `@`, tmux session, agent session, checkpoint bookmark, and place in the revision graph. A thread can follow another thread. When the parent changes, jj may evolve descendant commits immediately in repository state; kiki waits for a safe boundary before materializing that evolved state in the child's files and telling the agent what changed. When the parent's live head gains a new commit that jj cannot infer the child should follow, kiki explicitly rebases the child's provably linear owned stack at that same boundary.

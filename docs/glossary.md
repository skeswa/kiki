...
Definitions of the load-bearing terms in kiki's specs and architecture. When a term is used in multiple places, this is the canonical definition.
...
- **Bookmark** — jj's term for a named pointer to a revision. In kiki, the thread's bookmark is the human-facing handle and the thing pushed to GitHub as a branch (since the repo is colocated jj+git). The stable sqlite `thread_id`, not the bookmark, is kiki's canonical identity.
...
- **`cascade_outbox`** — per-(thread, applied_cascade_seq) row carrying the synthetic payload composed at the decision step. Pins the payload across crash + retry boundaries so the visible `thread_messages` row (written inside the `MarkDelivered` handler, conditional on actual delivery) cannot be a phantom for content the agent never saw. See [`decisions/cascade-outbox.md`](decisions/cascade-outbox.md).
...
- **`anchor_unknown=TRUE`** — flag on a backfilled transcript row whose timestamp predates the available `op_history` and cannot be reliably anchored to a `(change_id, commit_id, op_id)`. Anchor-aware queries skip these rows by default; `--recent` and FTS5 search include them. See [`decisions/transcript-anchoring.md`](decisions/transcript-anchoring.md).
...
- **PreToolUse hook** — the Claude Code hook point kiki uses to intercept tool calls for cascade delivery. Implemented by `kk-hook`. PostToolUse is deliberately not used because Claude Code does not fire it for tools blocked by PreToolUse.
...
- **`pending_kkd_prepends`** — TTL-bounded sidecar table the JSONL projector consults to recognize kiki-composed `--resume` prepended messages so they are captured as `synthesized=TRUE` rather than misattributed to the human.
...
- **Causal chain** — UUID stamped on each MCP-driven post, used to detect cycles, depth excess, and branch fan-out in inter-thread messaging. See [`roadmap/mcp-v2.md`](roadmap/mcp-v2.md).

...
- **`anchor_unknown=TRUE`** — flag on a backfilled transcript row whose timestamp predates the available `op_history` and cannot be reliably anchored to a `(change_id, commit_id, op_id)`. Anchor-aware queries skip these rows by default; `--recent` and FTS5 search include them. See [transcript anchoring](../appendix/decisions/transcript-anchoring.md).
